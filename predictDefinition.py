...
if TEST_BY_SENTENCE:
    if WRITE_TEST:
        from data.contextSentRawFull import wsddata as contexts
    else:
        from data.contextSentRaw import wsddata as contexts
else:
    if WRITE_TEST:
        from data.contextDataRawFull import wsddata as contexts
    else:
        from data.contextDataRaw import wsddata as contexts
        ...
        for i in range(len(definition_glob) - 1):
            ...
            for j in range(len(fdefinition)-1):
                ...
                if token == currf:
                    ...
                    if next_token == nextf:
                        score += POINTS_FOR_CONSECUTIVE_WORD
                    # if current word is flagged as being a context word, award additional points
                    if next_token == "<context_word>":
